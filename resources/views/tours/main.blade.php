        ...
        <div class="row min-vh-75 align-items-center">
            <div class="col-12 d-block d-lg-none text-center mb-4">
                <h3 class="hero-title fs-1">
                    Discover <span class="text-gradient">Unforgettable</span><br>Andaman Family
                    Tour Packages
                </h3>
            </div>
            <div class="col-lg-8 d-none d-lg-block hero-content">
                <div class="animate-fade-in">
                    <div class="badge-wrapper">
                        <span class="hero-badge">{{$category->name}} Tour Experience</span>
                    </div>

                    <h1 class="hero-title">
                        Discover <span class="text-gradient">Unforgettable</span> <br>
                        {!! preg_replace('/-/', '<br>', $category->title, 1) !!}
                    </h1>
...
<section>
    <div class="container">
        <div class="tour-andamanbliss-section mt-3">
            @php
            $parts = explode('|', $category->categorySection[0]->title, 2);
            @endphp
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable tour-andamanbliss-modal modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="andamanblissModalLabel">{{$category->categorySection[0]->modal_title}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-0">
                    <div class="tour-andamanbliss-packages m-0">
                        <div class="tour-andamanbliss-packages-header">
                            <h3>Andaman {{ucwords($category->name)}} Packages</h3>
                        ...
                        <div class="tour-andamanbliss-package-item">
                            <div class="tour-andamanbliss-package-name">
                                <a href="{{ route('tour.static' ,['slug' => $tour['tourCategory']->slug, 'subslug' => $tour->slug]) }}">Andaman
                                    {{$tour->tourCategory->name}} Package</a>
                            </div>
                            <div class="tour-andamanbliss-package-duration">{{$tour->nights}} Nights & {{$tour->days}} Days</div>
                            <div class="tour-andamanbliss-package-inclusion">Hotels, Ferry, Cab, Sightseeing, Entry Fees
                            </div>
                            <div class="tour-andamanbliss-package-price">INR {{number_format($tour->start_price,2)}} ~PP</div>
                        </div>
                        @endforeach
...
<section id="package-options" class="packages-section">
    <div class="container">
        <div class="section-header text-center">
            @php
            $blocks = json_decode($category->categorySection[0]->blocks_section, true);
            $section1 = $blocks['section_1'] ?? null;
            $section2 = $blocks['section_2'] ?? null;

            @endphp

            @if($section1)
            <span class="section-subtitle">{{ $section1['badge'] ?? '' }}</span>
            <h2 class="section-title">
                {!! preg_replace('/\|/', '<span class="text-gradient">', $section1['heading'], 1) !!}
                    @if(str_contains($section1['heading'], '|'))
                </span>
                @endif
            </h2>
            <p class="section-description">{{ $section1['description'] ?? '' }}</p>
            @endif
        ...
        <div class="packages-grid">
            ...
            @if($tours)
            @foreach ($tours->where('best_seller', 1)->take(3) as $tour)
            <div class="package-card" data-aos="fade-up" data-aos-delay="100">
                <div class="package-image">
                    <img src="{{ $tour->tourPhotos[0]->file }}" alt="3 Nights 4 Days Package">
                    <div class="package-tag">Most Popular</div>
                </div>
                <div class="package-content">
                    <h3 class="package-title">{{ucwords($tour->package_name)}} Package </h3>
                    <div class="package-price-family">
                        <span class="price">₹{{ number_format($tour->package_cost , 0) }}</span>
                        <span class="per-person">per person</span>
                    </div>
                    <p class="package-description"> {{ \Illuminate\Support\Str::words(ucfirst(strip_tags($tour->description)), 20, '...') }}</p>
                    <ul class="package-features">
                        <li><i class="fas fa-check-circle"></i>Port Blair & Havelock</li>
                        <li><i class="fas fa-check-circle"></i>Key Attractions</li>
                        <li><i class="fas fa-check-circle"></i>Essential Activities</li>
                    </ul>
                    <div class="package-footer">
                        <a href="{{ route('tour.static' ,['slug' => $tour['tourCategory']->slug, 'subslug' => $tour->slug]) }}" class="btn btn-primary">View
                            Details</a>
                    </div>
                </div>
        ...
        <section class="duration-section" id="duration-comparison">
            <div class="section-header text-center">
                @if($section2)
                <span class="section-subtitle">{{ $section2['badge'] ?? '' }}</span>
                <h2 class="section-title">
                    {!! preg_replace('/\|/', '<span class="text-gradient">', $section2['heading'], 1) !!}
                        @if(str_contains($section2['heading'], '|'))
                    </span>
                    @endif
                </h2>
                <p class="section-description">{{ $section2['description'] ?? '' }}</p>
                @endif
            </div>
            @php

            $smaller = collect($tours)->map(function ($tour) {
            return (int) substr($tour->package_name, 0, 1);
            })->min();

            $preferredDuration = $smaller;
            $sortedTours = $tours->sortBy(function($tour) use ($preferredDuration) {
            return [
            substr($tour->package_name, 0, 1) == $preferredDuration ? 0 : 1,
            $tour->id
            ];
            });
            @endphp
            ...
            <div class="duration-tabs">
                ...
                <button type="button" class="duration-tab {{ $loop->iteration == 1 ? 'active' : '' }}"
                    data-duration="{{ substr($tour->package_name, 0, 1) }}">
                    {{ substr($tour->package_name, 0, 1) }} Nights
                </button>
                @endforeach
                @endif
            </div>
            ...
            <div class="duration-content">
                @if($tours)
                @foreach ($sortedTours as $tour)
                <div class="duration-card" data-duration="{{ substr($tour->package_name, 0, 1) }}" data-aos="fade-up"
                    data-aos-delay="100">
                    <div class="duration-image">
                        <img src="{{ $tour->tourPhotos[0]->file }}"
                            alt="{{ substr($tour->package_name, 0, 1) }} Nights Package">
                        <div class="duration-badge">Most Popular</div>
                    </div>
                    <div class="duration-details">
                        <div class="duration-header">
                            <h3 class="duration-title">{{ ucwords($tour->tourCategory->name.' ' .$tour->package_name) }} Package</h3>
                            <div class="duration-price">₹{{number_format($tour->package_cost,0)}} <small>per
                                    person</small></div>
                        </div>
                        <div class="duration-features">
                            <div class="duration-feature">
                                <i class="fas fa-check"></i>
                                <span>Perfect for short getaways</span>
                            </div>
                            <div class="duration-feature">
                                <i class="fas fa-check"></i>
                                <span>Cover major attractions</span>
                            </div>
                            <div class="duration-feature">
                                <i class="fas fa-check"></i>
                                <span>Port Blair city tour</span>
                            </div>
                            <div class="duration-feature">
                                <i class="fas fa-check"></i>
                                <span>Havelock Island visit</span>
                            </div>
                        </div>
                        <div class="duration-footer">
                            ...
                            <a href="{{ route('tour.static' ,['slug' => $tour['tourCategory']->slug, 'subslug' => $tour->slug]) }}"
                                class="duration-cta">
                                View Details
                            </a>
                        </div>
...
<section class="features-section" id="features">
    <div class="container">
        <div class="section-header text-center">
            <span class="section-subtitle">Why Choose Our Packages</span>
            <h2 class="section-title">Experience the <span class="text-gradient">Best of Andaman</span></h2>
            <p class="section-description">Our family-friendly packages are carefully designed to ensure a fun, safe,
                and memorable Andaman experience for all ages.</p>
        </div>
        ...
        <div class="features-grid">
            <div class="feature-card" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="feature-content">
                    <h3 class="feature-title">Exceptional Service:</h3>
                    <p class="feature-description">Personalized care ensures your family’s needs are met every step of
                        the way.</p>
                </div>
            ...
            <div class="feature-card" data-aos="fade-up" data-aos-delay="200">
                ...
                <div class="feature-content">
                    ...
                    <p class="feature-description">Enjoy exclusive activities and hidden gems with our Best Family Tour
                        Packages In Andaman.</p>
                </div>
                <div class="feature-bg"></div>
            </div>
            ...
            <div class="feature-card" data-aos="fade-up" data-aos-delay="300">
                ...
                <div class="feature-content">
                    ...
                    <p class="feature-description">Our deep island knowledge guides you to the best spots for a
                        memorable Family Friendly Trip To Andaman Islands.</p>
                </div>
                <div class="feature-bg"></div>
            </div>
            ...
            <div class="feature-card" data-aos="fade-up" data-aos-delay="400">
                ...
                <div class="feature-content">
                    <h3 class="feature-title">Customizable Packages:</h3>
                    <p class="feature-description">Tailor your itinerary to match your family’s unique preferences.</p>
                </div>
                <div class="feature-bg"></div>
            </div>
        </div>
...
<section class="faq-section" id="faq">
    <div class="container">
        <div class="section-header text-center">
            ...
            <p class="section-description">Find answers to common questions about our Andaman tour packages</p>
        </div>
            ...
            <div class="faq-accordion" id="faqAccordion">
                ...
                <div class="faq-container">
                    <div class="row">
                        ...
                        $left = $faqs->slice(0, $half);
                        $right = $faqs->slice($half);
                        $delay = 100;
                        @endphp

                        <div class="col-md-6">
                            @foreach($left as $faq)
                            <div class="faq-item" data-aos="fade-up" data-aos-delay="{{ $delay }}">
                                <div class="faq-header" id="heading{{ $loop->iteration }}">
                                    <button class="faq-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#faq{{ $loop->iteration }}">
                                        <span class="faq-title">{{ $faq->question }}</span>
                                        <span class="faq-icon"><i class="fas fa-plus"></i><i class="fas fa-minus"></i></span>
                                    </button>
                                </div>
                                <div id="faq{{ $loop->iteration }}" class="collapse" data-bs-parent="#faqAccordion">
                                    <div class="faq-body">
                                        <p>{!! $faq->answer !!}</p>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="faq-item" data-aos="fade-up" data-aos-delay="{{ $delay }}">
                                <div class="faq-header" id="heading{{ $loop->iteration + $half }}">
                                    <button class="faq-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#faq{{ $loop->iteration + $half }}">
                                        <span class="faq-title">{{ $faq->question }}</span>
                                        <span class="faq-icon"><i class="fas fa-plus"></i><i class="fas fa-minus"></i></span>
                                    </button>
                                </div>
                                <div id="faq{{ $loop->iteration + $half }}" class="collapse" data-bs-parent="#faqAccordion">
                                    <div class="faq-body">
                                        <p>{!! $faq->answer !!}</p>
                                    </div>
                                </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        tabs.forEach(tab => {
            tab.addEventListener("click", function() {
                ...
                cards.sort((a, b) => {
                    const aMatch = a.getAttribute("data-duration") === selectedDuration ?
                        0 : 1;
                    const bMatch = b.getAttribute("data-duration") === selectedDuration ?
                        0 : 1;
